...
const receiptSlice = createSlice({
  ...
  initialState,
  reducers: {
    setCart: (state: any, action: PayloadAction<Receipt | null>) => {
      state.cart = action.payload;
    },
    setReceipt: (state: any, action: PayloadAction<Receipt[]>) => {
      state.receipts = action.payload;
    },
    setAll: (state: any, action: PayloadAction<Receipt[]>) => {
    ...
    update: (state: any, action: PayloadAction<Receipt>) => {
      state.receipts = state.receipts.map((item: any) =>
        item.id == action.payload.id ? action.payload : item
      );
    },
    addReceipt: (state: any, action: PayloadAction<Receipt>) => {
